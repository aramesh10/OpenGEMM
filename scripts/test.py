...
def check(name, m, n, k, emit, workdir):
    entry = bench.make_inputs(m, n, k, name)
    if DTYPES[name].impl == "smm":
        operands = (entry.a, entry.b, entry.sfa, entry.sfb)
    else:
        operands = (entry.a, entry.b)
    names = {"atype": name} if entry.a.dtype == torch.uint8 else {}
    if emit:
    ...
    torch.testing.assert_close(out, bench.reference(entry, name), rtol=rtol, atol=atol)
...
def main():
    ...
    parser.add_argument(
        "--emit", action="store_true", help="go through emit_kernel + run_kernel instead"
    )
        ...
        for name in names:
            for m, n, k in SHAPES:
                ...
                try:
                    ...
                    failed += 1
                    print(
                        f"FAIL  {name:10s} {m}x{n}x{k}: {str(exc).splitlines()[0][:120]}",
                        flush=True,
                    )

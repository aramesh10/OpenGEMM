...
// (supergroup, walk, split count, L2 promotion, K padding) is a launch
// argument, stored beside it and read at runtime. Adding a line widens every
// future sweep. Removing one narrows it, and a stored shape that named it
// stops launching: the error prints this list.
...
//   (cta_group, block_m, mma_n, stages, swap_ab, epi_hold, epi_mode,
//    epi_direct, use_clc, split_k, rm, rn, rk)
...
// Per operand pair: the plain 128- and 256-wide tiles at one and two CTAs,
// the narrow swapped tiles decode shapes take, deep rings, cluster launch
// control, split-K (a separate instantiation, since the accumulating
// epilogue is compiled in), the dual-MMA epilogue (epi_mode = 1) on f16 and
// e4m3, and the 64-row block on e4m3.
using All = List<
    // bf16

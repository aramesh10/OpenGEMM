...
OG_API int32_t     og_smm_abi(void)     { return OG_ABI_VERSION; }
OG_API const char *og_smm_error(void)   { return error_text; }
OG_API int32_t     og_smm_kernels(void) { return KERNELS; }

OG_API int32_t og_smm_kernel(int32_t i, OgGemm *out) {
  if (i < 0 || i >= KERNELS)
    return opengemm::fail(OG_ERR_BIND, "kernel %d is out of range; og_smm_kernels() is the count", i);
  smm_registry::describe(POLICIES[i], out);
  out->row = i;
  return OG_OK;
}

OG_API int32_t og_smm_bind(OgGemm *g) { return bind(g); }

OG_API int32_t og_smm_launch(const OgGemm *g, const void *a, const void *b,
